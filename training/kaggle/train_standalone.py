#!/usr/bin/env python3
"""Self-contained LLM pretraining script for Kaggle kernels.

All model/data/optimizer code is embedded inline so the kernel
only needs the pre-tokenized binary dataset.

Architecture: 68.7M param decoder-only Transformer with RoPE, SwiGLU, RMSNorm.
"""
...
def main():
    print("=" * 60)
    print("LLM PRETRAINING — KAGGLE GPU (self-contained)")
    print("=" * 60)

    is_kaggle = os.path.exists("/kaggle/working")
    if is_kaggle:
        CORPUS_DIR = Path("/kaggle/input/research-v2-corpus")
    ...
    config = {
        ...
        "training": {"optimizer": "muon", "learning_rate": 3e-4, "muon_learning_rate": 0.02,
                     "min_lr": 3e-5, "weight_decay": 0.1, "momentum": 0.95, "grad_clip": 5.0,
    ...
    if not Path(tokens_path).exists():
        ...
        print("  Please run pre-tokenization first.")
        sys.exit(1)
    ...
    for f in sorted(ckpt_dir.glob("step_*.pt"), key=lambda p: p.stat().st_mtime):
        try:
            ckpt = load_checkpoint(str(f), model, optimizer, scheduler, scaler, device)
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if (batch_idx + 1) % grad_accum == 0:
                ...
                if global_step % save_every == 0:
                    ...
                    save_checkpoint(str(path), model, optimizer, scheduler, scaler, ema=ema,
                ...
                if global_step % eval_every == 0:
                    ...
                    metrics = evaluate(model, val_loader, device, max_steps=train_cfg.get("eval_steps", 50))
                    ...
                    if metrics["val_loss"] < best_val_loss:
                        ...
                        save_checkpoint(str(ckpt_dir / "best.pt"), model, optimizer, scheduler, scaler, ema=ema,
                                       step=global_step, total_tokens=total_tokens_count,
                                       extra={"best_val_loss": best_val_loss})
                        print(f"    New best: {best_val_loss:.4f}")
                    model.train()

    save_checkpoint(str(ckpt_dir / "final.pt"), model, optimizer, scheduler, scaler, ema=ema,
                   step=global_step, total_tokens=total_tokens_count,
                   extra={"best_val_loss": best_val_loss})
    ...
    try:
        ...
        tokenizer = Tok()
        for i, (p, s) in enumerate(zip(prompts, generate_samples(model, tokenizer, prompts,
